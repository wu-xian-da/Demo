...
<body>
		    ...
		    <div class="swiper-wrapper">
		        <c:forEach items="${imgNewsList }" var="imgNews">
		        	 <div class="swiper-slide">
		        	    <a href="${base }/news/toDetail/${imgNews.id}">
	...
	<div class="navigation">
	    <c:forEach items="${navList }" var="nav">
	        ...
	        <a href="${base }/nav/toList/${nav.id}">		
	        	</c:when>
	        	<c:when test="${not empty nav.navType and 'WEJCD' eq nav.navType}">
	        <a href="${base }/info/toContent/${nav.id}">	
	        	</c:when>
			...
			<div class="weather-tips">
				<ul>
					<c:forEach items="${noticeList }" var="notice">
						<li><a href="${base }/notice/toDetail/${notice.id }">【紧急通告】${notice.title }</a></li>
					</c:forEach>
				</ul>
			</div>
			
			<a href="${base }/notice/toList" class="more">更多>></a>
		</div>
	</div>
